...
public class DimSwapToBTD {
    ...
    public static void onAdvancement(ServerPlayer player, Advancement advancement) {
        if (advancement.getId().equals(new ResourceLocation("true_end:stop_dreaming"))) {
            ...
            if (!TEVariables.getPlayerData(player).getBeenBeyond())
                if (player.level().dimension() == Level.OVERWORLD
                        ...
                        Objects.requireNonNull(player.server.getAdvancements()
                                .getAdvancement(new ResourceLocation("true_end:stop_dreaming")))).isDone()) {
            ...
            for (int x = -searchRadius; x <= searchRadius; x++) {
                for (int z = -searchRadius; z <= searchRadius; z++) {
                    ...
                    if (level.getBlockState(candidate).is(TEBlocks.GRASS_BLOCK.get())
                            && level.getBiome(candidate).is(new ResourceLocation("true_end:plains"))
                        ...
                        TrueEndCommon.LOGGER.info("Found ideal spawn: {}", above);
                        return above;
                    }
                }
            }
            ...
            for (int x = -searchRadius; x <= searchRadius; x++) {
                for (int z = -searchRadius; z <= searchRadius; z++) {
                    ...
                    if (level.getBlockState(candidate).is(TEBlocks.GRASS_BLOCK.get())
                            ...
                            && level.isEmptyBlock(above)
                            && isValidSpawnArea(level, candidate)) {
                        TrueEndCommon.LOGGER.info("Found fallback spawn: {}", above);
                        return above;
                    }
                }
            }
    ...
    private static void sendFirstEntryConversation(ServerPlayer player) {
        ...
        try {
            ...
            TrueEndCommon.LOGGER.error("Error reading {} with exception {}", file, e);
            return;
        } finally {
            try { br.close(); } catch (Exception ignored) {} }

        //Play text
        TrueEndCommon.queueServerWork(45, () -> {
            TrueEndCommon.messageWithCooldown(player, jsonLines.toArray(new String[0]), TEConfig.btdConversationDelay);
        });
    }
}